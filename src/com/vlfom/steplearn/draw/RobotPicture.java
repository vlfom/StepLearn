...
public class RobotPicture {
    ...
    public Rectangle2D.Double getBodyCoords() {
        return new Rectangle2D.Double(bodyCoords.x - robot.body.width/2.0,
                bodyCoords.y - robot.body.height/2.0, robot.body.width, robot
                .body.height);
    }
    ...
    public Line2D.Double getTibCoords(int id) throws
        ...
        return new Line2D.Double(bodyCoords.x, bodyCoords.y - robot.body
                .height / 2.0, bodyCoords.x + leg.tib.length * Math.cos(leg
                .tib.angle), bodyCoords.y - robot.body.height / 2.0 - leg.tib
                .length * Math.sin(leg.tib.angle));
    }
    ...
    public Line2D.Double getFootCoords(int id) throws
        ...
        Point2D.Double center = new Point2D.Double(bodyCoords.x + leg.tib
                .length * Math.cos(leg.tib.angle), bodyCoords.y - robot.body
                .height / 2.0 - leg.tib.length * Math.sin(leg.tib.angle));
        return new Line2D.Double(center.x + leg.foot.length / 2.0 * Math.cos
                (leg.foot.angle - leg.tib.angle), center.y + leg.foot.length
                / 2.0 * Math.sin(leg.foot.angle - leg.tib.angle), center.x -
                leg.foot.length / 2.0 * Math.cos(leg.foot.angle - leg.tib
                        .angle), center.y - leg.foot.length / 2.0 * Math.sin
                (leg.foot.angle - leg.tib.angle));
    }
}